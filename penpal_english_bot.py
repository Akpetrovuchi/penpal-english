...
PERSONA_PROMPTS = {
    "interview": "You are a hiring manager conducting a short job interview. Your name is Max. Speak as a polite, professional manager in English. Introduce yourself very briefly (1 sentence) and then ask a specific interview question to start. Be concise.",
    ...
    "raise": "You are the user's manager. Your name is Max. Start the conversation professionally in English: introduce yourself very briefly (1 sentence), ask why the employee thinks they deserve a raise.",
...
# Stickers for each roleplay topic (sent at session start)
ROLEPLAY_STICKERS = {
    "travel": "CAACAgIAAxkBAAOHaS8Ig_zfxNDIdX7NA1eQ2cB2DIYAAhOKAAJ-OnlJV4f8vhIywpw2BA",
    "raise": "CAACAgIAAxkBAAOKaS8IwIQBKaBSr9Fxp1BPpGX7iOUAAguQAAK7FHlJdp3KJm9E-1Y2BA",
    "restaurant": "CAACAgIAAxkBAAOMaS8IwxQ3JqUc3uv6rZ3VJ0j84QcAAmeLAAKJgnhJc-1neYBs0zQ2BA",
    "free": "CAACAgIAAxkBAAOOaS8IzrOslVNmBwABbHf6XzuYDNliAALmigACvzl5SVcSAAGcGnuUizYE",
    "interview": "CAACAgIAAxkBAAOQaS8I2lYisIIoAkWSYXsKoDetIlwAAmaPAAIPB3lJwXTA57oN8bw2BA",
}
...
async def choose_chat_topic(c: types.CallbackQuery):
    ...
    # Send sticker first (if available for this topic)
    sticker_id = ROLEPLAY_STICKERS.get(topic_key)
    if sticker_id:
        try:
            await bot.send_sticker(user_id, sticker_id)
        except Exception:
            logging.exception(f"Failed to send sticker for topic {topic_key}")
...
async def menu_main_callback(c: types.CallbackQuery):
    ...
    # Update streak first (resets streak_notified_today if new day)
...
def update_streak(user_id):
    ...
    Also resets streak_notified_today if this is first activity of the day.
    """
    today = date.today()
    with closing(db()) as conn:
        c = conn.cursor()
        ...
        # Update DB with streak_notified_today reset
        c.execute("""
            UPDATE users 
